...
/**
 * @title DepositManagerBaseSepoliaForkTest
 * @notice Fork tests against Base Sepolia testnet to verify USDC and Aave V3 integration
 * @dev Run with: forge test --match-contract DepositManagerBaseSepoliaForkTest --fork-url $BASE_SEPOLIA_RPC_URL -vvv
 */
contract DepositManagerBaseSepoliaForkTest is Test {
    ...
    function setUp() public {
        // Skip if not forking Base Sepolia (chainId 84532)
    ...
    function test_Fork_Sepolia_USDCExists() public onlyFork {
        assertEq(usdc.decimals(), 6);
    }

    function test_Fork_Sepolia_UserHasUSDC() public onlyFork {
        assertEq(usdc.balanceOf(user), 10_000 * 1e6);
    }

    // ==================== Aave Pool Tests ====================

    function test_Fork_Sepolia_AavePoolExists() public onlyFork {
    ...
    function test_Fork_Sepolia_AaveUSDCReserveActive() public onlyFork {
    ...
    function test_Fork_Sepolia_DepositUSDC() public onlyFork {
    ...
    function test_Fork_Sepolia_WithdrawUSDC() public onlyFork {
        uint256 depositAmount = 100 * 1e6; // $100 USDC
    ...
    function test_Fork_Sepolia_DepositWithdrawCycle() public onlyFork {
        ...
        // User should have approximately the same USDC
        uint256 finalBalance = usdc.balanceOf(user);
        assertGe(finalBalance, initialBalance - 1);
    }

    function test_Fork_Sepolia_MultipleDeposits() public onlyFork {
    ...
    function test_Fork_Sepolia_ViewFunctions() public onlyFork {
